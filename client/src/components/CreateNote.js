...
class CreateNote extends Component {
  ...
  handleChange(content, delta, source, editor) {
    ...
    this.setState({textValue: aText, textHtml: content, updated_date: oTime});
    var that = this;
    ...
    setInterval(function() {
      ...
      if (!that.state.saved) {
        ...
        axios.post('/notes', {
          ...
          that.setState({note_id: result.data._id, saved: true});
          //this.props.history.push("/")
        });
      } else {
        //debugger;
        if (Object.keys(that.oTextDelta).length !== 0 && that.oTextDelta.length() > 0) {
          axios.put('/notes/' + that.state.note_id, {
            textValue,
            titleValue,
            textHtml,
            titleHtml,
  ...
  handleTitleChange(content, delta, source, editor) {
    ...
    var oTime = moment().format('lll');
    this.setState({titleValue: sTitle, titleHtml: content, updated_date: oTime});
    var that = this;
  ...
  render() {
    return (<div className="container-fluid custom-app">
      ...
      <Row>
        ...
        <ReactQuill className="quill-title" theme="bubble" placeholder="Add title" value={this.state.titleHtml || " "} onChange={this.handleTitleChange}></ReactQuill>
        <ReactQuill id="quill" className="quill-editor" theme="bubble" placeholder="Add note" value={this.state.textHtml || " "} onChange={this.handleChange}></ReactQuill>
      ...
      <Row>
        <div>
          ...
          <Modal show={this.state.show} onHide={this.handleClose}>
            ...
            <Modal.Body>
              {
                this.state.aPO.map((po) => <div>Update Purchase Order
              ...
              {
                this.state.aNewQt.map((qt) => <div>Create new Sales quotaion
                  <Table striped="striped" bordered="bordered" condensed="condensed" hover="hover">
                    <thead>
                      <tr>
                        <th>Fields</th>
